...
<div id="queuePreview" class="queue-preview" hidden aria-hidden="true">
  <span class="queue-preview-label">Up next</span>
  <div class="queue-preview-row">
    <div class="queue-preview-avatar-wrap">
      <img class="queue-preview-avatar" alt="" />
      <span class="queue-preview-avatar-fallback" hidden></span>
    </div>
    <span class="queue-preview-name"></span>
  </div>
</div>
...
(function () {{
  ...
  var queuePreviewEl = null;
  var previewAvatarEl = null;
  var previewAvatarFallbackEl = null;
  var previewNameEl = null;

  function getQueuePreviewElements() {{
    if (!queuePreviewEl) {{
      queuePreviewEl = document.getElementById('queuePreview');
      if (queuePreviewEl) {{
        previewAvatarEl = queuePreviewEl.querySelector('.queue-preview-avatar');
        previewAvatarFallbackEl = queuePreviewEl.querySelector('.queue-preview-avatar-fallback');
        previewNameEl = queuePreviewEl.querySelector('.queue-preview-name');
      }}
    }}
    return queuePreviewEl;
  }}

  function previewDisplayName(person) {{
    if (isUnknownPerson(person) || person.employee_found === false) {{
      return 'USER NOT FOUND';
    }}
    return (person.name || person.email || '').toUpperCase();
  }}

  function fillPreviewAvatar(person) {{
    if (!previewAvatarEl || !previewAvatarFallbackEl) return;
    var letter = ((person.name || person.email || '?').charAt(0) || '?').toUpperCase();
    previewAvatarFallbackEl.textContent = letter;
    previewAvatarFallbackEl.hidden = true;
    previewAvatarEl.hidden = false;
    previewAvatarEl.alt = previewDisplayName(person);
    var avatarUrl = resolveAvatarUrl(person);
    if (!avatarUrl) {{
      previewAvatarEl.hidden = true;
      previewAvatarFallbackEl.hidden = false;
      return;
    }}
    previewAvatarEl.onload = function () {{
      previewAvatarEl.hidden = false;
      previewAvatarFallbackEl.hidden = true;
    }};
    previewAvatarEl.onerror = function () {{
      previewAvatarEl.hidden = true;
      previewAvatarFallbackEl.hidden = false;
    }};
    previewAvatarEl.src = avatarUrl;
    if (previewAvatarEl.complete) {{
      if (previewAvatarEl.naturalWidth > 0) {{
        previewAvatarEl.onload();
      }} else {{
        previewAvatarEl.onerror();
      }}
    }}
  }}

  function updateQueuePreview() {{
    if (!getQueuePreviewElements()) return;
    if (queue.length === 0) {{
      queuePreviewEl.hidden = true;
      queuePreviewEl.setAttribute('aria-hidden', 'true');
      return;
    }}
    var next = queue[0];
    if (previewNameEl) {{
      var nameText = previewDisplayName(next);
      if (queue.length > 1) {{
        nameText += ' (+' + (queue.length - 1) + ' waiting)';
      }}
      previewNameEl.textContent = nameText;
    }}
    fillPreviewAvatar(next);
    queuePreviewEl.hidden = false;
    queuePreviewEl.setAttribute('aria-hidden', 'false');
    liveLog('queue preview', next.id, queue.length);
  }}
  ...
  function showNext() {{
    if (currentLive || queue.length === 0) {{
      if (queue.length === 0) updateQueuePreview();
      return;
    }};
    if (!slideshow) {{
      slideshow = document.getElementById('slideshow');
